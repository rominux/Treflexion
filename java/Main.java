...
class Main extends Program {
    ...
    boolean PoserQuestion() {
        ...
        while (!valid) {
            ...
            if (!valid) {
                msgErreur = ANSI_RED + "Réponse invalide (A, B, C ou D attendu)" + ANSI_RESET;
            }
        }
        ...
        String msgResultat = "";
        if (correct) {
            msgResultat = ANSI_GREEN + "BONNE RÉPONSE ! +1 Joker" + ANSI_RESET;
        } else {
            msgResultat = ANSI_RED + "MAUVAISE RÉPONSE... (" + q.bonneReponse + ")" + ANSI_RESET;
        }
    ...
    // Affiche l'écran principal du jeu avec la grille, les messages et les infos (utilise jeu.txt).
    ...
    // Fonction vide pour compatibilité future (placement manuel).
