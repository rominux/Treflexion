...
class Main extends Program {   
    ...
    String dernierCoup = ""; // NOUVEAU : Pour stocker le dernier mouvement
    ...
    void LancerJeu() {
        ...
        derniereExplication = "";
        dernierCoup = "Début de partie"; // Reset
        String messageInfo = "";
                ...
                if (lig == -3) {
                    ...
                    if (col == -3) {
                        ...
                        if (grille[lig][col] == null) {
                            ...
                            // NOUVEAU : Enregistrement du coup
    ...
    // ========================================================================================================================
    // AFFICHAGE (CORRIGÉ POUR ÉVITER LE DÉCALAGE)
    // ========================================================================================================================
    ...
    void AfficherEcranJeu(Carte[][] grille, Carte cActuelle, Carte cSuivante, String message, int nbJokers) {
        ...
        // 5. Affichage du Dernier Coup Joué (Ligne 44)
        if (length(dernierCoup) > 0) {
            EcrireDansBuffer(ecran, 45, 67, ">> " + dernierCoup);
        }

        // 6. Affichage du message d'information/erreur en COULEUR (Ligne 45 = Index 45 du tableau si on compte 0)
        // Correction : Ligne 46 visuelle = Index 45
        if (length(message) > 0) {
    ...
    // Calculer la longueur d'une chaine sans compter les caractères invisibles (codes couleurs)
    int longueurVisuelle(String s) {
        int len = 0;
        boolean inAnsi = false;
        for (int i = 0; i < length(s); i++) {
            char c = charAt(s, i);
            if (c == '\u001B') { // Début d'un code ANSI
                inAnsi = true;
            } else if (inAnsi && c == 'm') { // Fin standard d'un code couleur ANSI
                inAnsi = false;
            } else if (!inAnsi) {
                len++;
            }
    ...
    // Nouvelle version qui utilise longueurVisuelle pour ne pas décaler le décor
    void EcrireDansBuffer(String[] ecran, int l, int c, String texte) {
        if (l >= length(ecran)) return;
        String ligne = ecran[l];
        
        int lenV = longueurVisuelle(texte); // On utilise la longueur VISUELLE, pas la longueur informatique

        if (c + lenV > length(ligne)) return;
        
        // On coupe la ligne originale : Début + Texte inséré + Fin (mais on ne coupe la fin qu'après la longueur visuelle)
        ecran[l] = substring(ligne, 0, c) + texte + substring(ligne, c + lenV, length(ligne));
    }
    
    // ... [Reste des fonctions utilitaires inchangées ci-dessous] ...
